    ...
    <div className="bg-white rounded-xl border border-app-border shadow-xs overflow-hidden">
      ...
      <div className="px-6 pt-5 pb-4 border-b border-slate-100 bg-slate-50/50">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-mono px-2 py-0.5 bg-white border border-slate-200 text-slate-600 rounded">
              {analysis.model}
            </span>
          </div>
        </div>
        {/* Summary as the lead paragraph — the anchor for everything below */}
      ...
      <div className="p-6 space-y-6">
          ...
          <section className="rounded-xl border border-emerald-100 bg-emerald-50/40 p-5 space-y-3">
            ...
            <ul className="space-y-2.5">
              {analysis.keyFindings.map((finding, idx) => (
                <li key={idx} className="text-sm text-slate-800 leading-relaxed flex items-start gap-2.5">
        ...
        {analysis.hypotheses && analysis.hypotheses.length > 0 && (
          <section className="rounded-xl border border-brand-100 bg-brand-50/30 p-5 space-y-3">
            ...
            <div className="space-y-3">
              {analysis.hypotheses.map((h, idx) => (
                <div key={idx} className="p-4 bg-white border border-brand-100 rounded-lg space-y-2 shadow-2xs">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <span className="text-sm font-medium text-slate-900 leading-relaxed flex-1 min-w-[16rem]">
                      {h.statement}
                    </span>
                      ...
                      {h.supportingObservationIds.map((obsId) => {
                        ...
                        return (
                          <Link
                            ...
                            className="text-brand-700 bg-brand-50/60 hover:bg-brand-100 px-1.5 py-0.5 rounded border border-brand-100 transition"
                          >
                            {summary?.title || obsId}
                          </Link>
                        );
        ...
        {analysis.uncertainties && analysis.uncertainties.length > 0 && (
          <section className="rounded-xl border border-amber-200/70 bg-amber-50/50 p-5 space-y-3">
            ...
            <ul className="space-y-2.5">
              {analysis.uncertainties.map((unc, idx) => (
                <li key={idx} className="text-sm text-amber-900 leading-relaxed flex items-start gap-2.5">
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50/60 p-5 space-y-3">
            ...
            <ul className="space-y-2.5">
              {analysis.suggestedQuestions.map((q, idx) => (
                <li key={idx} className="text-sm text-slate-800 leading-relaxed flex items-start gap-2.5">
                  <ArrowRight className="w-4 h-4 text-brand-500 mt-[3px] shrink-0" />
                  <span>{q}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="space-y-2.5">
              {analysis.suggestedNextSteps.map((step, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex items-center justify-between gap-4 p-4 bg-white border border-app-border rounded-lg transition-colors hover:border-brand-200 shadow-2xs"
                  >
                    <span className="text-sm text-slate-800 leading-relaxed flex-1">{step}</span>
                    {isAccepted ? (
                      <span className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-md shrink-0">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-white bg-brand-600 hover:bg-brand-700 disabled:opacity-50 rounded-md transition-colors shadow-xs shrink-0"
                      >
